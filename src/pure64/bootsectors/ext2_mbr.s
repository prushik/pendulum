...
;
; This Master Boot Record will load Pure64 from a sector map on the
; hard drive without making use of the file system.
;
; We have abandoned BMFS. Which means we no longer know the location of the
; kernel on disc. This code will load sector 2 as a psuedo second stage, and
; use the block list it contains to load the rest of the kernel.
; =============================================================================
...
;	mov ebx, 32			; Start immediately after directory (offset 8192)
;	mov cx, 0x8000		; Pure64 expects to be loaded at 0x8000

	mov eax, 1			; load 1 sector
	mov ebx, 1			; second half of mbr
	mov cx, bootmap		; load it directly after mbr in memory
	call readsector		; cool, we did it

	mov edx, bootmap

	lea ecx, [0x8000]	; we do want to load into 0x8000 (Pure64 still wants to start here)
...
msg_Load db "ext2 mbr v1.0 - Loading Pure64", 0
...
;This is the second sector of the bootloader. 
bootmap:

; if we don't get overwritten, maybe we are still using bmfs?
db 64,0,0,0,32,0,0,0
