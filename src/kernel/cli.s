...
os_command_line:
	...
	call os_output
	mov ebx, 0x0055FF55		; Green text in graphics mode
	mov [os_Font_Color], ebx
	...
	mov rdi, cls_string		; 'clear' entered?
	call os_string_compare
	jc near clear_screen

	mov rdi, dir_string		; 'ls' entered?
	call os_string_compare
	jc near ls

	mov rdi, cd_string		; 'cd' entered?
	call os_string_compare
	jc near cd

	mov rdi, stat_string		; 'stat' entered?
	call os_string_compare
	jc near stat

	mov rdi, cat_string		; 'cat' entered?
	call os_string_compare
	jc near cat

	mov rdi, ext_string		; 'ext' entered? this is for test porpoises
	call os_string_compare
	jc near ext
...
;	lea rsi,[directory]
;	mov cl,[directory+ext2_directory.name_size]
...
cd:
	...
	mov rdx, 2					;
	mov rax, 1
	call os_system_config		; argv[1]

	call os_ext2_search_directory ; search for 
...
ext: ; this is just for testing, and it gets removed when ext2 is ready
...
;MiB_MSG: db ' MB', 13, 0
;List_MSG: db 'Name                            | Size (Byte)        | Reserved (MB) ', 13, \
;		'--------------------------------------------------------------------------', 13, 0
; -----------------------------------------------------------------------------
